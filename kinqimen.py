...
class Qimen:
    ...
    def hourganghzi_zhifu(self):
        gz = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        return config.multi_key_dict_get(dict(zip(list(map(lambda x: tuple(x),
                                         list(map(lambda x: config.new_list(config.jiazi(), x)[0:10],
                                                  config.jiazi()[0::10])))),
                                         list(map(lambda x: config.jiazi()[0::10][x] +
                                                  config.tian_gan[4:10][x],
                                         list(range(0,6)))))), gz[3])
    #分值符
    ...
    def pan_earth_minute(self):
        return dict(zip(list(map(lambda x: dict(zip(config.cnumber, config.eight_gua)).get(x),
                        config.new_list(config.cnumber, config.qimen_ju_name_ke(self.year, self.month, self.day, self.hour, self.minute)[2]))),
                        {"陽遁":list("戊己庚辛壬癸丁丙乙"),"陰遁":list("戊乙丙丁癸壬辛庚己")}.get(
                            config.qimen_ju_name_ke(self.year, self.month, self.day, self.hour, self.minute)[0:2])))
    #逆地盤
    def pan_earth_r(self, option):
        return dict(zip(list(self.pan_earth(option).values()), list(self.pan_earth(option).keys())))
    ...
    def pan_sky(self, option):
        qmju = {1:config.qimen_ju_name_chaibu(self.year, self.month, self.day, self.hour, self.minute),
                2:config.qimen_ju_name_zhirun(self.year, self.month, self.day, self.hour, self.minute)}.get(option)
        rotate = {"陽":config.clockwise_eightgua,
                  "陰":list(reversed(config.clockwise_eightgua))}.get(qmju[0])
        zhifu_n_zhishi = config.zhifu_n_zhishi(self.year, self.month, self.day, self.hour, self.minute, option)
        ...
        if fu_head_location == "中":
            ...
            try:
                gan_reorder, gong_reorder= config.new_list(list(map(lambda x: self.pan_earth(option).get(x), list(rotate))), fu_head),
                config.new_list(rotate,  fu_head_location)
                return dict(zip(gong_reorder, gan_reorder))
            except ValueError:
                return dict(zip(list(reversed(gong_reorder)),
                                config.new_list(list(map(lambda x: self.pan_earth(option).get(x), list(reversed(rotate)))), self.pan_earth(option).get("坤"))))
        if fu_head_location != "中" and zhifu != "禽" and fu_head_location2 != "中":
            ...
            if fu_head in gan_reorder:
                if fu_location == None:
                    ...
                    return {**dict(zip(gong_reorder,gan_reorder)),**{"中":self.pan_earth(option).get("中")}}
        if fu_head_location != "中" and zhifu == "禽" and fu_head_location2 == "中":
            gan_reorder = config.new_list(list(map(lambda x: self.pan_earth(option).get(x), list(rotate))), self.pan_earth(option).get("坤"))
            gong_reorder = config.new_list(rotate,  fu_head_location)
            if fu_head not in gan_reorder:
                rgong_reorder = config.new_list(gong_reorder , fu_location)
                return dict(zip(rgong_reorder, gan_reorder))
            if fu_head in gan_reorder:
                return {**dict(zip(gong_reorder,gan_reorder)),**{"中":self.pan_earth(option)[0].get("中")}}
    #天盤分
    def pan_sky_minute(self, option):
        rotate = {"陽":config.clockwise_eightgua,"陰":
                  list(reversed(config.clockwise_eightgua))}.get(
                      config.qimen_ju_name_ke(self.year, self.month, self.day, self.hour, self.minute)[0]
                      )
        gz = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        fu_head = self.hourganghzi_zhifu_minute()[2]
        fu_location = self.pan_earth_min_r().get(gz[4][0])
        fu_head_location = config.zhifu_n_zhishi_ke(self.year, self.month, self.day, self.hour, self.minute, option).get("值符星宮")[1]
        fu_head_location2 = self.pan_earth_min_r().get(fu_head)
        zhifu = config.zhifu_n_zhishi_ke(self.year, self.month, self.day, self.hour, self.minute, option)["值符星宮"][0]
        if fu_head_location == "中":
            gong_reorder = config.new_list(rotate, "坤")
            try:
                gan_reorder, gong_reorder= config.new_list(list(map(lambda x: self.pan_earth(option).get(x), list(rotate))), fu_head),
        ...
        if fu_head_location != "中" and zhifu != "禽" and fu_head_location2 != "中":
            ...
            if fu_head not in gan_reorder:
                start = dict(zip(config.cnumber, gan_reorder)).get(
                    config.qimen_ju_name(self.year, self.month, self.day, self.hour, self.minute)[2])
                rgan_reorder = config.new_list(gan_reorder , start)
                rgong_reorder = config.new_list(gong_reorder , fu_location)
                return dict(zip(rgong_reorder, rgan_reorder)), dict(zip(rgan_reorder, rgong_reorder))
            if fu_head in gan_reorder:
    ...
    def pan(self, option):#1拆補 #2置閏
        ...
        pan_star_result = config.pan_star(self.year, self.month, self.day, self.hour, self.minute, option)
        star = pan_star_result[0]
        door = config.pan_door(self.year, self.month, self.day, self.hour, self.minute, option)
        god = config.pan_god(self.year, self.month, self.day, self.hour, self.minute, option)
        return {
    ...
    def pan_minute(self, option):
        ...
        paiju = qmju
        j_q = config.jq(self.year, self.month, self.day, self.hour, self.minute)
        zfzs = config.zhifu_n_zhishi_ke(self.year, self.month, self.day, self.hour, self.minute, option)
        pan_star_result = config.pan_star_minute(self.year, self.month, self.day, self.hour, self.minute, option)
        star = pan_star_result[0]
        door = config.pan_door_minute(self.year, self.month, self.day, self.hour, self.minute, option)
        god = config.pan_god_minute(self.year, self.month, self.day, self.hour, self.minute, option)
        return {
            "排盤方式":{1:"拆補", 2:"置閏"}.get(option),
            "干支": gzd,
            "旬首": shunhead,
    ...
    def pan_html(self, option):
        god, door, star, sky, earth = self.pan_god(option), self.pan_door(option), self.pan_star(option)[0],  self.pan_sky(), self.pan_earth()
    ...
    def gpan(self):
        ...
        yy = {"冬至":"陽遁", "夏至":"陰遁"}.get(config.multi_key_dict_get(
            {tuple(config.jieqi_name[0:12]):"冬至", tuple(config.jieqi_name[12:24]):"夏至"},
            config.jq(self.year, self.month, self.day, self.hour, self.minute)))
        gong = dict(zip(start_jia, {"冬至": "艮離坎坤震巽", "夏至":"坤離巽坤離兌"}.get(config.multi_key_dict_get(
            {tuple(config.jieqi_name[0:12]):"冬至", tuple(config.jieqi_name[12:24]):"夏至"}, j_q
            )))).get(shun)
        ...
        ying = dict(zip(config.new_list(config.eight_gua,
                                        {**dict(zip(close_ten_day, config.new_list(list(reversed(config.eight_gua)), gong))),
                                         **{close_ten_day[-1]:a_gong[0]}}.get(dgz)), config.golen_d))
        yang = dict(zip(config.new_list(config.eight_gua,
                                        {**dict(zip(close_ten_day, config.new_list(config.eight_gua, gong))),
                                         **{close_ten_day[-1]:a_gong[0]}}.get(dgz)), config.golen_d))
    ...
    def tianyi(self, option):
        zhifu_n_zhishi= config.zhifu_n_zhishi(self.year, self.month, self.day, self.hour, self.minute, option)
        try:
            star_location = dict(zip(config.eight_gua, list("蓬芮沖輔禽心柱任英"))).get(zhifu_n_zhishi.get("值符星宮")[1])
        except IndexError:
            star_location = "禽"
        return star_location
    #丁馬
    def dinhorse(self):
        Gangzhi = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        new_dict = dict(zip(re.findall("..","甲子甲戌甲申甲午甲辰甲寅"), list("卯丑亥酉未巳")))
        new = config.multi_key_dict_get(config.liujiashun_dict(), Gangzhi[2])
        return config.multi_key_dict_get(new_dict, new)
    #天馬
    def moonhorse(self):
        Gangzhi = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        new = list(map(lambda i:tuple(i), re.findall("..","寅申卯酉辰戌巳亥午子丑未")))
        new_dict = dict(zip(new, list("午申戌子寅辰")))
        return config.multi_key_dict_get(new_dict, Gangzhi[2][1])
    #驛馬星
    def hourhorse(self):
        Gangzhi = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        new = list(map(lambda i:tuple(i), re.findall("...","申子辰寅午戌亥卯未巳酉丑")))
        new_dict = dict(zip(new, list("寅申巳亥")))
        return config.multi_key_dict_get(new_dict, Gangzhi[3][1])

    def overall(self, option):
        return {"金函玉鏡(日家奇門)": self.gpan(), "時家奇門": self.pan(option), "刻家奇門":self.pan_minute(option)}
...
if __name__ == '__main__':
    ...
    print(Qimen(2024,5,5,18,46).pan(2))
