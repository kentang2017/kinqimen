...
class Qimen:
    ...
    def qimen_ju_day(self):
        ju_day_dict = {tuple(list("甲己")):"甲己日",  tuple(list("乙庚")):"乙庚日",
                       tuple(list("丙辛")):"丙辛日", tuple(list("丁壬")):"丁壬日",
                       tuple(list("戊癸")):"戊癸日"}
    ...
    #地盤
    def pan_earth(self):
        qm_ju = config.qimen_ju_name(self.year, self.month, self.day, self.hour, self.minute)[2]
        return dict(zip(list(map(lambda x: dict(zip(config.cnumber, config.eight_gua)).get(x),
                                 config.new_list(config.cnumber, qm_ju))),
                                {"陽遁":list("戊己庚辛壬癸丁丙乙"),"陰遁":list("戊乙丙丁癸壬辛庚己")}.get(
                                    config.qimen_ju_name(self.year, self.month, self.day, self.hour, self.minute)[0:2]))
                                    )
    #地盤
    ...
    def pan_sky(self, option):
        rotate = {"陽":config.clockwise_eightgua, 
                  "陰":list(reversed(config.clockwise_eightgua))}.get(
                      config.qimen_ju_name(self.year, self.month, self.day, self.hour, self.minute)[0]
                      )
        ...
        fu_head_location = config.zhifu_n_zhishi(self.year, self.month, self.day, self.hour, self.minute, option).get("值符星宮")[1]
        fu_head_location2 = self.pan_earth_r().get(fu_head)
        zhifu = config.zhifu_n_zhishi(self.year, self.month, self.day, self.hour, self.minute, option)["值符星宮"][0]
        if fu_head_location == "中":
            gong_reorder = config.new_list(rotate, "坤")
            try:
                gan_reorder, gong_reorder= config.new_list(list(map(lambda x: self.pan_earth().get(x), list(rotate))), fu_head),
                ...
                return dict(zip(list(reversed(gong_reorder)), 
                                config.new_list(list(map(lambda x: self.pan_earth().get(x), list(reversed(rotate)))), self.pan_earth().get("坤")) )
                            )
    ...
    def pan(self, option):#1拆補 #2置閏
        ...
        paiju = config.qimen_ju_name(self.year, self.month, self.day, self.hour, self.minute, option)
    ...
    def pan_zhirun(self, option): #1拆補 #2置閏
        # Calculate gangzhi once and store the result for reuse
        gz = config.gangzhi(self.year, self.month, self.day, self.hour, self.minute)
        gzd = "{}年{}月{}日{}時".format(gz[0], gz[1], gz[2], gz[3])
        shunhead = config.shun(gz[2])
        shunkong = config.daykong_shikong(self.year, self.month, self.day, self.hour, self.minute)
        paiju = config.qimen_ju_name_zhirun(self.year, self.month, self.day, self.hour, self.minute)[1]
        j_q = config.jq(self.year, self.month, self.day, self.hour, self.minute)
    ...
    def tianyi(self, option):
        try:
            star_location = dict(zip(config.eight_gua, list("蓬芮沖輔禽心柱任英"))).get(config.zhifu_n_zhishi(self.year, self.month, self.day, self.hour, self.minute, option).get("值符星宮")[1])
        except IndexError:
            star_location = "禽"
    ...
    def overall(self, option):
        return { "金函玉鏡(日家奇門)": self.gpan(), "時家奇門": self.pan(option), "刻家奇門":self.pan_minute()}
...
if __name__ == '__main__':
    ...
    print(Qimen(2024,12,21,14,0).pan_zhirun(1))
